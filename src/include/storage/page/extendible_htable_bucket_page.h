...
class ExtendibleHTableBucketPage {
 ...
 private:
  ...
  auto Key2Idx(const KeyType &key, const KeyComparator &cmp) const -> int;
  auto GetOneSlot() -> int;
  void PutOneSlot(int idx);

 public:
  void RemoveAll();
  void InsertWithoutCheck(const KeyType &key, const ValueType &value);
};
